...
const redAlertField = document.querySelector('.message');
...
searchBtn.addEventListener('click', function (event) {
    ...
    if (trimSearch.length > 0) {
        ...
        showAlert('please give valid Input  Search!');
    }
    searchInput.value = '';
});
...
const spinner = (isShow = true) => {
    if (isShow) {
        searchBtn.innerHTML = `
            <span class="spinner-load spinner-load-sm" role="status" aria-hidden="true"></span>
...
const showAlert = (redAlert = '') => {
    if (redAlert.length > 0) {
        redAlertField.innerHTML = `<h6 class="d-inline-block m-0 bg-info text-danger  py-2 px-3 rounded">${redAlert}</h6>`;

        // Remove Message After 3000 MS
        setTimeout(() => {
            redAlertField.innerHTML = '';
        }, 3000);
    } else {
        redAlertField.innerHTML = '';
    }
